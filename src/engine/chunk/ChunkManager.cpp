...
Engine::Chunk::ChunkManager::ChunkManager()
    : m_voxelTexture(Engine::TextureManager::get("voxel")) {
  ...
  void Engine::Chunk::ChunkManager::generateChunkData(
  ...
  void Engine::Chunk::ChunkManager::processChunkUpdates() {
    ...
    while (m_chunkUpdateQueue.try_dequeue(chunkUpdate)) {
      ...
      if (chunkUpdate->newData != nullptr) {
        auto &inUse =
            gApp.getRegistry().get<Engine::InUse>(chunkUpdate->entity);
        if (inUse.removeUser()) {
          gApp.getRegistry().remove<Engine::InUse>(chunkUpdate->entity);
        }
  ...
  void Engine::Chunk::ChunkManager::render(const glm::mat4 &viewProjection) {
    ...
    auto &camera =
        gApp.getRegistry().get<Engine::Camera>(gApp.getActiveCamera());
