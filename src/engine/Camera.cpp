...
glm::mat4 update(entt::registry &registry, float aspectRatio) {
  ...
  for (auto entity : view) {
    ...
    if (transform.roll != 0.0) {
      glm::mat4 rollMatrix =
          glm::rotate(glm::mat4(1.0f), rollRad, camera.front);
      camera.up = glm::normalize(
          glm::vec3(rollMatrix *
                    glm::vec4(glm::cross(camera.right, camera.front), 0.0f)));
    } else {
    ...
    camera.projection =
        glm::perspective(glm::radians(camera.fov), aspectRatio, 0.1f, 1024.0f);
...
void update(entt::registry &registry, float dt) {
  ...
  for (auto entity : view) {
    ...
    transform.position +=
        wishDir * cameraController.movementSpeed * dt * 100.0f;
  }
}
} // namespace Engine::CameraControllerSystem
